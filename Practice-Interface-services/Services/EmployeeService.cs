...
namespace Practice_Interface_Services.Services
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public EmployeeResponse GetById(Employee[] employees, int? id)
        {
            if(id == null)
            {
                return new EmployeeResponse { ErrorMessage = EmployeeResponseMessage.ArgumentNull };
            }
            return new EmployeeResponse { Employee = employees.FirstOrDefault(m => m.Id == id) };

            if(employees is null)
            {
                return new EmployeeResponse { ErrorMessage = EmployeeResponseMessage.NotFound };
            }
